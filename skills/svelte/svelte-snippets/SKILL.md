...
description: >
  Guides to passing nested content into Svelte 5 components using `{@render...}` and `{#snippet...}`. Use when implementing or refactoring components that use snippets, `{#snippet ...}`, `{@render ...}`, or nested reusable content. Or when the user mentions snippets or slots.
paths:
  - "**/*.svelte"
  - "**/*.svelte.**"
---
